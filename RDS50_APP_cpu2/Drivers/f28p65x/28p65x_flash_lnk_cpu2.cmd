MEMORY
{
   ...
   -------------------------------------------------------*/
   RAMGS_PROG       : origin = 0x014000, length = 0x006000
   ...
   -------------------------------------------------------*/
   FLASH_ALL_SEC  : origin = 0x0E0002, length = 0x03FFFE
...
{
   codestart        : > BEGIN
   .text            : >> FLASH_ALL_SEC, ALIGN(8)
   .cinit           : > FLASH_ALL_SEC, ALIGN(8)
   .switch          : > FLASH_ALL_SEC, ALIGN(8)
   .reset           : > RESET, TYPE = DSECT /* not used, */

   .stack           : > RAMM1
#if defined(__TI_EABI__)
   .bss             : > RAMGS_DATA
   .bss:output      : > RAMGS_DATA
   .init_array      : > FLASH_ALL_SEC, ALIGN(8)
   .const           : > FLASH_ALL_SEC, ALIGN(8)
   .data            : > RAMGS_DATA
   .sysmem          : > RAMGS_DATA
#else
   .pinit           : > FLASH_ALL_SEC, ALIGN(8)
   .ebss            : >> RAMGS_DATA
   .econst          : > FLASH_ALL_SEC, ALIGN(8)
   .esysmem         : > RAMGS_DATA
#endif

   ramgs0 : > RAMGS_DATA, type=NOINIT
   ramgs1 : > RAMGS_DATA, type=NOINIT
   ramgs2 : > RAMGS_DATA, type=NOINIT

   MSGRAM_CPU1_TO_CPU2 > CPU1TOCPU2RAM, type=NOINIT
   MSGRAM_CPU2_TO_CPU1 > CPU2TOCPU1RAM, type=NOINIT

   #if defined(__TI_EABI__)
       .TI.ramfunc : {} LOAD = FLASH_ALL_SEC,
                        RUN = RAMGS_PROG,
