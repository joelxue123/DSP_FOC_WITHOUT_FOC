 ...
 * *
 ...
 dt = 0.125  4HZ,半周期0.125ms
 J = Te / ( dw / dt) = 3.3572e-05
 现在有了转动惯量，可以去求解 自抗扰观测器了。
 * *
 */
...
{
    if (velocity == 0) {
        return 0; // 静态摩擦力，方向未定义，通常在启动时考虑
    } else {
        // 动态摩擦力，方向与速度方向相反
        if (velocity > 20) {
            return 0.3f; // 负载向右移动，摩擦力向左
        } else if (velocity < -20) {
            return -0.3; // 负载向左移动，摩擦力向右
        }
    }
    return 0; // 速度为0时，摩擦力不明确，通常不计入动态控制
}
...
void initFilter(IIRLowPassFilter *filter, float32 cutoffFreq, float32 sampleRate) {
    filter->alpha = 1.0f - exp(-2.0f * 3.1415f * cutoffFreq / sampleRate); // 计算alpha值
...
// ADRC参数
#define WC   150.0f
#define WO  (5*WC)     /* 观测器带宽 */
#define B0  (1*KT/J)      /* 控制增益 */
#define KP  (WC*WC)    /* PD控制器P参数 */
#define KD  (2*WC)     /* PD控制器D参数 */
#define BETA1  (3.0f*WO)
#define BETA2  (3*WO*WO)
#define BETA3  (WO*WO*WO)
#define DT  0.0001f    /* 执行时间步长 */
...
interrupt void CIPC1_INT_isr(void)
{
  ...
  { //数据传输必须在50us任务开始传输
      ...
      ipcCPU2ToCPU1Data.Reserve1 = 0.0f-TorqueSensorDiffAd_pu;// ipcCPU1ToCPU2Data.TargetTorque_Nm*1000/7; //监控4
      ipcCPU2ToCPU1Data.Reserve2 = xx22;//xx22;//Task50usTime.CycleTime_us;//监控5
